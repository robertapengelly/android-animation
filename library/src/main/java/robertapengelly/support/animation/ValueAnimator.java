 ...
 *
 * <p>There is a single timing pulse that all animations use. It runs in a custom handler to ensure that property changes happen on the UI thread.</p>
 *
 * <p>By default, ValueAnimator uses non-linear interpolation, via the {@link AccelerateDecelerateInterpolator} class,
...
public class ValueAnimator extends Animator {
    ...
    /** Internal per-thread collections used to avoid set collisions as animations start and end while being processed. */
    ...
    public void cancel() {
        ...
        if ((mPlayingState != STOPPED) || sDelayedAnims.get().contains(this) || sPendingAnimations.get().contains(this)) {
    ...
    /**
     * Returns the interpolator that this ValueAnimator uses.
     *
     * @return The interpolator for this ValueAnimator.
     */
    ...
    /**
     * Returns the values that this ValueAnimator animates between. These values are stored in PropertyValuesHolder objects,
     * even if the ValueAnimator was created with a simple list of value objects instead.
     *
     * @return PropertyValuesHolder[] An array of PropertyValuesHolder objects which hold the values, per property, that define the animation.
     */
     ...
     * If there is a nonzero <code>startDelay</code>, the function is called after that delay ends.
     * It takes care of the final initialization steps for the animation.
     *
     * <p>Overrides of this method should call the superclass method to ensure that internal mechanisms for the animation are set up correctly.</p>
     */
    void initAnimation() {
     ...
     *                  provide the ncessry interpolation between the Object values to derive the animated value.
     * @param values    A set of values that the animation will animate between over time.
     *
     * @return A ValueAnimator object that is set up to animate between the given values.
     */
    ...
    /**
     * Sets float values that will be animated between. A single value implies that that value is the one being animated to.
     * However, this is not typically useful in a ValueAnimator object because there is no way for the object to determine the
     * starting value for the animation (unlike ObjectAnimator, which can derive that value
     * from the target object and property being animated). Therefore, there should typically
     * be two or more values.
     *
    ...
    /**
     * Sets int values that will be animated between. A single value implies that that value is the one being animated to.
     * However, this is not typically useful in a ValueAnimator object because there is no way for the object to determine the
     * starting value for the animation (unlike ObjectAnimator, which can derive that value
     * from the target object and property being animated). Therefore, there should typically
     * be two or more values.
    ...
    /**
     * Sets the values to animate between for this animation. A single value implies that that value is the one being animated to.
     * However, this is not typically useful in a ValueAnimator object because there is no way for the object to determine the
     * starting value for the animation (unlike ObjectAnimator, which can derive that value
     * from the target object and property being animated). Therefore, there should typically
     * be two or more values.
     ...
     * be constructed without values and this method can be called to set the values manually instead.
     *
    ...
    private void start(boolean playBackwards) {
        ...
        if (Looper.myLooper() == null)
            throw new AndroidRuntimeException("Animators may only run on Looper threads");
